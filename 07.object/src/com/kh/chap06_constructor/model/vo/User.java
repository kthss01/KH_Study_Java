...
public class User {
	...
	// 기본 생성자 --> public 클래스명() {....}, 매개변수가 없는 생성자
	public User() {
		// 단지 객체 생성을 목적으로 할 때 사용
		// 기본 생성자를 생략하는 경우가 오류가 아니다 --> jvm이 자동으로 만들어 줬기 때문에
		// 항상 객체 생성이 가능했던 것
...
//		this.userName = userName;
		...
		// 위와 같이 중복되는 동일한 초기화 하는 내용이 생성자가 존재하는 경우 this() 생성자를 사용 가능
		// 같은 클래스 내에서는 생성자에서 달느 생성자를 호출 가능,
		// 단, 이때 반드시 첫 줄에 작성해야 한다. (단 한번만 호출 가능)
	 ...
	 * 오버로딩 : 
	 * 동일한 메소드명을 사용하는 것,
	 * 매개변수의 자료형과 갯수, 순서가 다르게 작성되어야 한다 (접근제한자, 반환형은 상관 없음)
	 */
