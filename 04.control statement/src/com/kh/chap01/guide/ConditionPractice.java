...
public class ConditionPractice {
	...
	public void practice9() {
		...
		double mid = midTerm * 0.2;   // 사용자가 입력한 중간고사 점수를 20% 반영
		double fin = finalTerm * 0.3; // 사용자가 입력한 기말고사 점수를 30% 반영
		double hw = homework * 0.3;   // 사용자가 입력한 과제점수를 30% 반영
		double at = attendance;		  // 사용자가 입력한 출석 횟수를 100% 반영 (왜? 출석횟수가 20회만점 기준으로 입력할 것이기 때문!)
		
		double sum = mid + fin + hw + at; // 위의 값들을 모두 더해서 총 100점 만점 기준으로 만듬
		
		System.out.println("===========결과==========");
		
		if(sum >= 70 && at >= 20 * 0.7) { // 1_1. 총점이 70점 이면서 출석횟수도 20회 만점 기준으로 70% 이상일 경우
